...
def initialize_auth_config():
    ...
    try:
        ...
        required_fields = [
            ...
            "guild_id", "role_ids", "jwt_secret", "frontend_url"
        ]
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if token_response.status_code != 200:
                logger.error(f"Discord token请求失败 - 状态码: {token_response.status_code}")
                logger.error(f"响应内容: {token_result}")
                logger.error(f"使用的redirect_uri: {_AUTH_CONFIG['redirect_uri']}")
                error_msg = token_result.get('error_description', token_result.get('error', '获取访问令牌失败'))
            ...
            member_response = await client.get(
                f"https://discord.com/api/users/@me/guilds/{_AUTH_CONFIG['guild_id']}/member",
                headers=headers
            )
            ...
            response = RedirectResponse(
                url=f"{_AUTH_CONFIG['frontend_url']}#token={token}",
                status_code=302
            )
